...
public static class DbInitializer
{
    ...
    public static void Seed(ServiceDeskDbContext context)
    {
        ...
        // Each section is guarded independently so clearing one table doesn't re-trigger others.
        try
        {
            // Always ensure system roles and admin user exist
            ...
            if (!context.CompanyServices.Any())
            {
            ...
            {
                // Only seed tickets if employees exist with the expected IDs
                var empIds = context.Employees.Select(e => e.Id).ToHashSet();
                if (empIds.Count >= 8)
                {
                    var eList = context.Employees.OrderBy(e => e.Id).Select(e => e.Id).ToList();
                    int e1 = eList[0], e2 = eList[1], e3 = eList[2], e4 = eList[3],
                        e5 = eList[4], e6 = eList[5], e7 = eList[6], e8 = eList[7];

                    var tickets = new Ticket[]
                    {
                        new() { Title = "Cannot connect to VPN",                    Description = "Unable to establish VPN connection from home. Getting timeout errors.",                                                                       Category = TicketCategory.NetworkIssue,    Status = TicketStatus.Open,     Priority = TicketPriority.High,     CreatedDate = DateTime.UtcNow.AddDays(-2),  SubmittedById = e4, AssignedToId = e2 },
                        new() { Title = "Laptop screen flickering",                 Description = "Dell laptop screen flickers intermittently, especially when on battery power.",                                                               Category = TicketCategory.HardwareIssue,   Status = TicketStatus.InProgress,Priority = TicketPriority.Medium,   CreatedDate = DateTime.UtcNow.AddDays(-5),  SubmittedById = e5, AssignedToId = e3 },
                        new() { Title = "New employee onboarding - IT setup",       Description = "New hire starting next Monday in Marketing. Need full IT setup: laptop, email, Teams, required software.",                                    Category = TicketCategory.EmployeeIssue,   Status = TicketStatus.Open,     Priority = TicketPriority.High,     CreatedDate = DateTime.UtcNow.AddDays(-1),  SubmittedById = e6, AssignedToId = e3 },
                        new() { Title = "Email not syncing on mobile",              Description = "Outlook app on iPhone stopped syncing emails two days ago.",                                                                                   Category = TicketCategory.SoftwareIssue,   Status = TicketStatus.Resolved, Priority = TicketPriority.Low,      CreatedDate = DateTime.UtcNow.AddDays(-7),  UpdatedDate = DateTime.UtcNow.AddDays(-5), ResolvedDate = DateTime.UtcNow.AddDays(-5), SubmittedById = e8, AssignedToId = e3, ResolutionNotes = "Removed and re-added the email account. Syncing normally now." },
                        new() { Title = "Request access to ERP system",             Description = "Need access to the ERP system for financial reporting module.",                                                                                Category = TicketCategory.ServiceRequest,  Status = TicketStatus.Closed,   Priority = TicketPriority.Medium,   CreatedDate = DateTime.UtcNow.AddDays(-14), ResolvedDate = DateTime.UtcNow.AddDays(-12), ClosedDate = DateTime.UtcNow.AddDays(-10), SubmittedById = e5, AssignedToId = e1, ResolutionNotes = "ERP access granted with Finance Reader role." },
                        new() { Title = "Printer jamming frequently",               Description = "1st floor printer keeps jamming. Happens multiple times per day.",                                                                            Category = TicketCategory.HardwareIssue,   Status = TicketStatus.InProgress,Priority = TicketPriority.Medium,   CreatedDate = DateTime.UtcNow.AddDays(-3),  SubmittedById = e7, AssignedToId = e3 },
                        new() { Title = "Suspected phishing email received",        Description = "Received suspicious email claiming to be from CEO asking for wire transfer.",                                                                 Category = TicketCategory.SecurityIncident,Status = TicketStatus.Open,     Priority = TicketPriority.Critical, CreatedDate = DateTime.UtcNow,              SubmittedById = e8, AssignedToId = e1 },
                        new() { Title = "Software installation request - Visual Studio", Description = "Need Visual Studio 2024 Enterprise installed for development work.",                                                                    Category = TicketCategory.ServiceRequest,  Status = TicketStatus.Open,     Priority = TicketPriority.Low,      CreatedDate = DateTime.UtcNow.AddDays(-1),  SubmittedById = e4, AssignedToId = e2 },
                    };
